...
benchmark "makeBigData":
    for i in 0..1000_000:
        var key = $toMD5($i)
        objArr1.add MyObject1(
            key0: key,
            amount: random.rand(1000_000.00)
        )
        if i.mod(3) == 0:
...
benchmark "makeBigDataTable":
    for i in 0..1000_000:
        var key = $toMD5($i)
        df1[$i] = {
            "key0": Base_Type(kind: btk_string, string_val: key),
            "amount": Base_Type(kind: btk_float, float_val: random.rand(1000_000.00))
        }.toTable()
        if i.mod(3) == 0:
            df2[$i] = {
                ...
                "spent": Base_Type(kind: btk_float, float_val: random.rand(1000_000.00))
            }.toTable()
        elif i.mod(2) == 0:
            df2[$i] = {
                "key1": Base_Type(kind: btk_string, string_val: "key"),
